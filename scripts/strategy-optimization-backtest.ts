...
function cyclicalSignalExperiments(): Experiment[] {
  ...
  return [
    ...
    { name: "诊断_无熔断", question: "AI硬件诊断：去掉组合熔断（检验熔断是否在高波动下帮倒忙）", overrides: { exit_model: "profit_trail", drawdown_circuit: false, stop_streak_circuit: false } },
    { name: "诊断_缺口10", question: "AI硬件诊断：缺口上限放宽到10%", overrides: { ...base, max_open_gap_pct: 0.10 } },
    { name: "诊断_缺口20", question: "AI硬件诊断：缺口上限放宽到20%（几乎不设门禁）", overrides: { ...base, max_open_gap_pct: 0.20 } },
    { name: "诊断_止损15", question: "AI硬件诊断：技术止损放宽到-15%（检验波动错配）", overrides: { ...base, stop_loss_pct: 0.15 } },
    { name: "诊断_无转弱退出", question: "AI硬件诊断：换simple退出（无转弱/移动止损，检验洗盘问题）", overrides: { exit_model: "simple", drawdown_circuit: true, stop_streak_circuit: true } },
    { name: "缓冲_02", question: "抗洗盘缓冲：跌破MA10需超2%才转弱", overrides: { ...base, weakness_ma10_buffer: 0.02 } },
    { name: "缓冲_05", question: "抗洗盘缓冲5%", overrides: { ...base, weakness_ma10_buffer: 0.05 } },
    { name: "缓冲_08", question: "抗洗盘缓冲8%", overrides: { ...base, weakness_ma10_buffer: 0.08 } },
    { name: "缓冲_15", question: "抗洗盘缓冲15%", overrides: { ...base, weakness_ma10_buffer: 0.15 } },
    { name: "门禁_自适应", question: "门禁regime条件化：宽度>=0.6用5%、否则2%", overrides: { ...base, adaptive_open_gap: { strong: 0.05, weak: 0.02 } } },
    { name: "缓冲_自适应", question: "转弱缓冲regime条件化：宽度强容忍15%、宽度弱跌破即走——统一双轨的候选", overrides: { ...base, adaptive_weakness_buffer: { strong: 0.15, weak: 0 } } },
    { name: "缓冲自适应_门禁自适应", question: "两个regime条件化合并（最终形态候选）", overrides: { ...base,
      adaptive_weakness_buffer: { strong: 0.15, weak: 0 }, adaptive_open_gap: { strong: 0.05, weak: 0.02 } } },
    { name: "缓冲_05_门禁_自适应", question: "两个待验证方向合并", overrides: { ...base, weakness_ma10_buffer: 0.05, adaptive_open_gap: { strong: 0.05, weak: 0.02 } } },
