...
export function buildChatTools(deps: ChatToolDeps): AgentTool[] {
  ...
  const domainWriteSpecs: Array<{
  ...
  }> = [
    {
      ...
      label: "维护组合账户",
      description: "通过 record_position_change 记录买入、卖出、调整或备注，并逐事件固化决策来源、执行符合度、策略快照、可选计划与偏离原因；通过 upsert_account_snapshot 更新资金摘要。服务端统一经过组合账户 service；不能直接指定表或字段。",
    ...
    },
    {
      name: "pool_write",
      label: "维护标的池",
      description: "新增、更新、迁移或结束短线/长线池角色，并维护近期关注与官方行业展示顺序。新增必须完成角色、分级、评分、股性、阶段、标签和评估摘要；股票必须已有同花顺官方行业关系，不接受本地板块标签或自行指定行业字段。服务端保留历史角色行，同一标的只能有一个当前角色。",
  ...
  const domainWriteTools: AgentTool[] = domainWriteSpecs.map((spec) => ({
    name: spec.name,
    label: spec.label,
    description: `${spec.description} 确认制生成待确认提案；YOLO 模式直接事务执行。执行前重验目标状态，所有领域写工具共享数据库级写锁并写审计。`,
    ...
    execute: guard(deps, spec.name, spec.validate, async (params) =>
      withAgentMutationLock(pool, async (client) => {
        ...
        if (settings.yolo_mode) {
          ...
          return textResult(
            {
              message: `YOLO 模式已开启，${spec.label}已通过领域 service 执行`,
              mode: "yolo",
              preview: publicPreview,
              result,
            },
            { auto_approved: true, yolo_mode: true, payload: params, result },
          );
        }
  ...
  return [
    ...
    },
    {
      ...
      label: "编写并运行回测",
      description:
        "在隔离的临时 TypeScript 工作区验证一个策略思路，最多读取500000行日线。source_code 必须 default export async function run(sdk)，只能使用注入 sdk：sdk.codes、start、end、initialCash、parameters、bars(code)、stats.mean/stdev。返回 {daily_returns, metrics, conclusion, data_gaps}：daily_returns 必须有 1–50000 个唯一日期项 {date:'YYYY-MM-DD',return}；metrics 必须是最多100项的扁平对象，键匹配 ^[a-z][a-z0-9_]{0,62}$，值只能是有限数值或 null，结构化详情写入 conclusion；conclusion 为1–16000字符非空文本；data_gaps 为最多200项的数组。系统在无网络、无数据库凭据、只读根文件系统和资源限制的独立 Node 容器运行；代码、补丁、路径和 stdout/stderr 不进入数据库、消息或审计，结束后立即删除。失败会返回安全错误码、执行阶段和可用源码位置：STRATEGY_* 或回测结果契约错误应修正源码后最多自动重试一次，同一错误重复时停止；只有 WORKER_*、CONTAINER_* 或 INPUT_LIMIT 才表示容量或环境问题。可用 comparison_run_ids 关联历史思路与结论。",
      ...
      execute: guard<RunBacktestInput>(deps, "run_backtest", validateRunBacktestInput, async (params, context) => {
        ...
        return textResult({
          message: run.execution_status === "failed" ? "隔离回测失败；代码已删除" : "隔离回测已完成；代码已删除",
          run,
          source_code_persisted: false,
        });
      }),
