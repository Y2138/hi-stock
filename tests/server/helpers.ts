...
export async function seedTestStrategy(pool: pg.Pool, content = "# 测试当前策略"): Promise<void> {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO strategy_document (code, title, role, injection_order, content, sha256)
       VALUES ('test_strategy', '测试当前策略', 'portfolio', 10, $1, $2)`,
      [content, revisionHash],
    );
    // 清单哈希：与 repo.calculateStrategyHash 同公式（code:sha256 按注入序），
