...
当前计算版本命名为 `正式日线指标三版`，稳定标识同时写入代码常量和 run。日线输入固定为截至各标的最新行情日的近 6 年；更早行情继续保留，但不进入正式指标或复权可信度判定。MA/MACD 公式精确对齐正式引擎 `ta 0.11.0`，RSI14 使用 Wilder 口径：
...
RSI14：首个值使用 14 个相邻收盘涨跌额的平均涨幅与平均跌幅；之后分别按
alpha=1/14 做 Wilder 平滑。少于 15 根收盘价返回 NULL；全涨为 100、全跌为 0、
连续无涨跌为 50。
```
...
2. 在 repeatable read 中读取完整有序的正式窗口行情，计算输入 SHA-256。
3. 在事务外执行纯函数计算。
4. 新事务确认 dirty generation 未变化；变化时放弃本次结果并重排。
5. 原子替换当前值、写 run 并清 dirty。
6. 失败时写 failed run、保留 dirty 并退避重试。

每次始终对正式窗口全量复算，不从 `earliest_date` 接续 EMA 状态。规模证据证明必要前，不引入检查点优化。
